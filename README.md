...
------------------------------------------------------------------------------------------------------------------------------
6.迪米特法则(Demeter Principle)：一个实体应当尽量少的与其他实体之间发生相互作用，使得系统功能模块相对独立。
 应该使类与类之间尽量的使用直接关系，而尽量不使用非直接关系。
 类与类之间可以使用依赖，关联，组合，聚合等关系，但不能使用局部变的关系就是非直接关系。
 
 class Test{
   public void operation(){
      //TODO...
      
      A a = new A();//和A之间发生了耦合，如果直接使用该对象，则在对应的处理方式中，该整个method都处理每个业务发生关联，降低对应的关联系性，则
      可以提升对应的使用范围来确认对应的关系。
